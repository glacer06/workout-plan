...
document.getElementById('trainerTab').textContent = 'For ' + N.trainer;
...
var html = '';
if (!SESSION && S.tab === 'week') html += '<div class="card signin-nudge"><div><b>Sign in with Google</b><p class="fine">The private link is being retired. Sign in once on this phone to keep access.</p></div><div id="gsiNudge"></div></div>';
...
h += '</tbody></table></div>';
if (!S.editing && can()) h += '<p class="fine">Tap Edit plan at the top to fill in results.</p>';
h += '</section>';
return h;
}
function buildSheet() {
...
function loginCard() {
return '<div class="login-card"><img class="login-icon" src="steph-fitness-B-blue.svg" alt=""><div class="login-name">Sims Fitness</div><div id="gsiBtn" class="gsi-slot"></div><p class="fine" id="loginMsg"></p></div>';
}
...
return '<ol class="days"><li class="day' + (isToday ? ' is-today' : '') + '">' +
...
'<label class="field f-session"><span>Workout</span><select id="s-' + date + '" data-field="session" data-date="' + date + '"' + dis() + '>' + opts + '</select></label>' +
'<label class="field"><span>Weight</span><input id="w-' + date + '" data-field="weight" data-date="' + date + '" inputmode="decimal" placeholder="Optional" value="' + esc(e.weight == null ? '' : e.weight) + '"' + dis() + '></label>' +
'<button class="bonus" type="button" id="b-' + date + '" data-act="bonus" data-date="' + date + '" aria-pressed="' + (!!e.extra) + '"' + dis() + '>' + (e.extra ? 'Bonus: ' + esc(bonusList(e).join(' + ') || 'done') : 'Bonus move') + '</button>' +
'</div>' + liftsHTML(date, e) + '</li></ol>';
}
